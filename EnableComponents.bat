...
rem # Enable Anti-malware Service Executable (to restore run "sfc /scannow") 
...
ren "C:\ProgramData\Microsoft\Windows Defender\Platform\4.18.23070.1004-0\MsMpEng.exe.bak" "MsMpEng.exe"

rem # Enable INTEL Energy Server Service
...
ren "C:\Program Files\Intel\SUR\QUEENCREEK\x64\esrv_svc.exe.bak" "esrv_svc.exe"

rem # Enable WWAHost.exe (to restore run "sfc /scannow")
takeown /s %computername% /u %username% /f "%WinDir%\System32\WWAHost.exe"
icacls "%WinDir%\System32\WWAHost.exe" /grant:r %username%:F
ren "%WinDir%\System32\WWAHost.exe.bak" "WWAHost.exe"
